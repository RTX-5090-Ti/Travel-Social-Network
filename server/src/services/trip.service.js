...
import { buildHiddenTripExpiry } from "./hiddenTripCleanup.service.js";
...
export async function createTrip({
  ...
  try {
    ...
    return { tripId: tripDoc._id.toString() };
  } catch (err) {
...
export async function updateTrip({
  ...
  try {
    ...
    if (trip.ownerId.toString() !== userId?.toString()) {
      throw createHttpError(403, "Bạn không có quyền chỉnh sửa journey này.");
    }
...
export async function updateTripPrivacy({ tripId, userId, privacy }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }

  const trip = await Trip.findById(tripId).select("_id ownerId privacy updatedAt");
  ...
  if (trip.ownerId.toString() !== userId?.toString()) {
    throw createHttpError(
      403,
      "Bạn không có quyền chỉnh sửa đối tượng của journey này.",
    );
  }
...
export async function pinTrip({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  if (trip.ownerId.toString() !== userId?.toString()) {
    throw createHttpError(403, "Bạn chỉ có thể ghim journey của chính mình.");
  }
...
export async function unpinTrip({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  if (trip.ownerId.toString() !== userId?.toString()) {
    throw createHttpError(403, "Bạn chỉ có thể gỡ ghim journey của chính mình.");
  }
...
export async function saveTrip({ tripId, userId }) {
  ...
  if (!canView) {
    throw createHttpError(403, "Bạn không có quyền lưu journey này.");
  }

  const existing = await SavedTrip.findOne({ userId, tripId })
    .select("_id")
    .lean();

  if (!existing) {
    await SavedTrip.create({ userId, tripId });
  }
...
export async function unsaveTrip({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }

  await SavedTrip.deleteOne({ userId, tripId });
...
export async function hideTripForViewer({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  if (!canView) {
    throw createHttpError(403, "Bạn không có quyền ẩn journey này.");
  }

  if (trip.ownerId?.toString?.() === userId?.toString()) {
    throw createHttpError(403, "Bạn không thể ẩn journey của chính mình khỏi feed.");
  }
...
export async function moveTripToTrash({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  if (trip.ownerId.toString() !== userId?.toString()) {
    throw createHttpError(
      403,
      "Bạn chỉ có thể đưa chính journey của mình vào thùng rác.",
    );
  }
...
export async function restoreTripFromTrash({ tripId, userId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  if (trip.ownerId.toString() !== userId?.toString()) {
    throw createHttpError(403, "Bạn chỉ có thể khôi phục Journey của chính mình.");
  }
...
export async function listTrashedTrips({ userId, limit = 50 }) {
...
export async function listSavedTrips({ userId, limit = 50 }) {
...
export async function getTripDetail({ tripId, viewerId }) {
  if (!mongoose.isValidObjectId(tripId)) {
    throw createHttpError(400, "Invalid trip id");
  }
  ...
  const milestones = await Milestone.find({ tripId })
